<?php
    ...
    if($Q=="0" ){
		...
		if( !empty($_POST['code']) ){
			...
			/*file process*/
			$idnumber=fopen("bank_id.txt","r");
			flock("bank_id.txt", LOCK_EX);
			$id=fgets($idnumber);
			flock("bank_id.txt", LOCK_UN);
			fclose($idnumber);
		
			$changeid=fopen("bank_id.txt",'w');
			flock("bank_id.txt", LOCK_EX);
		...
		$insert = "INSERT INTO `student_grade`(`compileID`, `account`, `qid`, `response`,`grade`,`code`) VALUES ('".$id."','".$login."','".$Q."','".$response."','".$grade."','".$code."')";
