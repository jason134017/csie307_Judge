...
if (!isset($_COOKIE['account']) || $_COOKIE['identity']!="student" ) {
    echo "<script>alert('沒有權限登入,返回登入頁面');top.location.href ='../login/Homepage.html';</script>";	
}
?>
<!doctype html>
<html  lang="zh_tw">
...
$conn=mysqli_connect("localhost","root","csie307","Judge");
...
if (!$conn) 
           { die("資料庫意外: " . mysqli_connect_error()); 
	         ...
	         $query = "SELECT * FROM `student_account`". 
	                 " WHERE `student_account` LIKE '".$_COOKIE['account']."' ";  //帳號
		     $result = mysqli_query($conn,$query);	
	            //把撈出來的資料讀取出來
             $row_result=mysqli_fetch_assoc($result);
			 $account=$row_result['student_account'];
			...
			// echo "email:".$row_result["email"]."</br>";
             ...
             $sql = "SELECT * FROM `online_user` WHERE `account` LIKE '".$account."'";
