<?php
    ...
    if($Q=="0" ){
		...
		if( !empty($_POST['code']) ){
			...
			$sql = "SELECT * FROM `homework` WHERE `homework_id` LIKE '".$N."' ;";//查詢整個表單
			...
			if(strtotime($row['enddate'])<strtotime("now")){
				echo "<script>alert('".$N."超出時限');window.location.href ='homework.php';</script>";
			}
			...
			if ( $compiler==1 ){
				...
				echo "C++<br>";
				/*file process*/
				$exe='g++ /var/www/student/'.$id.'.cpp -o /var/www/student/'.$id.'.out -Wall ';
				shell_exec($exe);
				//測試
				/*echo "system<br>";
				...
				if (!fopen($output,"r")){
					...
					$errMsg=0;
					$move = 'rm -r /var/www/student/'.$id.'.cpp ';
					shell_exec($move);
				}else{
					for($i=1;$i<=5;$i++){
				...
				$exe='javac /var/www/student/Main.java ';
				shell_exec($exe);
				...
				if (!fopen($output,"r")){
					...
					echo '<font size=\'4\' color=\'#7700BB\'>CE:編譯錯誤</font>';
					$errMsg=0;
					$move = 'rm -r /var/www/student/Main.java ';
					shell_exec($move);
				}else{
					for($i=1;$i<=5;$i++){
						$filename="$Qid"."_$i"."_input.txt";
		...
		/*從伺服器刪除檔案*/
		$move = 'rm -r /var/www/student/'.$id.'.py ';
		shell_exec($move);
		$move = 'rm -r /var/www/student/Main.java ';
		shell_exec($move);
		$move = 'rm -r /var/www/student/Main.class ';
		shell_exec($move);
		////
        $move = 'rm -r /var/www/student/'.$id.'.cpp ';
		shell_exec($move);	
        $move2 ='rm -r /var/www/student/'.$id.'.out';
        shell_exec($move2);	
		for( $i=1;$i<=5;++$i){
			...
			$remove='rm -r /var/www/student/'.$answerf;
			$remove2='rm -r /var/www/student/'.$inputf;
			$remove3='rm -r /var/www/student/'.$outputf;
			$remove4='rm -r /var/www/student/'.$comparef;
		...
		$insert = "INSERT INTO `student_hwgrade`(`compileID`, `account`, `hwID`, `qid`, `response`,`grade`,`code`) VALUES ('".$id."','".$login."','".$N."','".$Q."','".$response."','".$grade."','".$code."')";
