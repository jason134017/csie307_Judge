...
<?php
	...
	if($login==""){
		...
		for( $i=1;$i<=($count['count(*)']-4);$i++ ){
			...
			mysqli_query($link, 'SET CHARACTER SET utf8'); 
			mysqli_query($link,  "SET collation_connection = 'utf8_general_ci'");
			$sql = "SELECT * FROM `student_examgrade` WHERE `account` LIKE '".$login."' AND `examID` LIKE '".$exam_id."' AND `qid` LIKE '".$qidArr[$i]."' ORDER BY `grade` DESC;";
			
			$result=mysqli_query($link,$sql);
			$row = mysqli_fetch_assoc($result);
			$response=$row['response'];	
				echo "<tr align='center'>
					<td><a href='Question_EXAM.php?qid=".$qidArr[$i]."&exam_id=".$exam_id."'>".$qidArr[$i]."</a></td>
