...
<?php
	...
	if($login!="teacher"){
		...
		for ( $i=1;$i<$t;$i++ ){
			...
			$sum=0;
			for ( $j=1;$j<$count;$j++ ){
				$sql = "SELECT * FROM `student_hwgrade` WHERE `hwID` LIKE '".$Q."' AND `qid` LIKE '".$qidArr[$j]."' AND `account` LIKE '".$studentArr[$i]."' ORDER BY `grade` DESC ; ";
				$result = mysqli_query($link,$sql);
				$row= mysqli_fetch_assoc($result);
	...
	echo "</table><br>";
	echo "<a href='homeworkgrade1.php?hw_id=".$Q."' >成績統計</a></br>";
?>
