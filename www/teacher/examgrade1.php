...
<?php
	...
	if($login==""){
		...
		$sql = "SELECT COUNT(DISTINCT `student_account`) AS count FROM `class`,`exam`,`student_examgrade` WHERE `exam_id` LIKE '".$Q."' AND `exam_id` LIKE `examID`
				AND `class`.`course_id` LIKE `exam`.`course_id` AND `class`.`student_account` LIKE `student_examgrade`.`account`;";//查詢整個表單
		...
		for ( $i=1;$i<$count;$i++ ){
			$sql = "SELECT COUNT(DISTINCT `student_account`) AS count FROM `class`,`exam`,`student_examgrade` WHERE `exam_id` LIKE '".$Q."' AND `exam_id` LIKE `examID`
					AND `class`.`course_id` LIKE `exam`.`course_id` AND `class`.`student_account` LIKE `student_examgrade`.`account` 
					AND `student_examgrade`.`response` LIKE 'AC' AND `student_examgrade`.`qid` LIKE '".$qidArr[$i]."'; ";
			mysqli_query($link, 'SET CHARACTER SET utf8'); 
			mysqli_query($link,  "SET collation_connection = 'utf8_general_ci'");
			$result = mysqli_query($link,$sql);
			$row= mysqli_fetch_assoc($result);
...
<?php
		...
		for ( $i=1;$i<$t;$i++ ){
			$sql = "SELECT COUNT(DISTINCT `qid`) AS count FROM `class`,`exam`,`student_examgrade` WHERE `exam_id` LIKE '".$Q."' AND `exam_id` LIKE `examID`
					AND `class`.`course_id` LIKE `exam`.`course_id` AND `class`.`student_account` LIKE `student_examgrade`.`account` 
					AND `student_examgrade`.`response` LIKE 'AC' AND `student_examgrade`.`account` LIKE '".$studentArr[$i]."'; ";
			mysqli_query($link, 'SET CHARACTER SET utf8'); 
			mysqli_query($link,  "SET collation_connection = 'utf8_general_ci'");
			$result = mysqli_query($link,$sql);
			$row= mysqli_fetch_assoc($result);
			...
			$sum = 0;
			for ( $j=1;$j<$count;$j++ ){
				$sql = "SELECT * FROM `student_examgrade` WHERE `examID` LIKE '".$Q."' 
