...
<?php
	...
	if($login==""){
		...
		$homework_id=$_POST["HW"];
		$homework_id=$homework_id[0];
		echo $homework_id;
		...
		for( $i=1;$i<=($count['count(*)']-4);$i++ ){
			...
			mysqli_query($link, 'SET CHARACTER SET utf8'); 
			mysqli_query($link,  "SET collation_connection = 'utf8_general_ci'");
			$sql = "SELECT * FROM `teacher_grade` WHERE `account` LIKE '".$login."' AND `ID` LIKE '".$homework_id."' AND `qid` LIKE '".$qidArr[$i]."' ORDER BY `grade` DESC;";
			
			$result=mysqli_query($link,$sql);
			$row = mysqli_fetch_assoc($result);
			$response=$row['response'];	
				echo "<tr align='center'>
					<td><a href='Question_HW.php?qid=".$qidArr[$i]."&homework_id=".$homework_id."' target='right'>".$qidArr[$i]."</a></td>
