...
<?php
	...
	$sql = "SELECT * FROM `homework` WHERE `course_id` LIKE '".$course."' ;";//查詢整個表單
	...
	while($row= mysqli_fetch_assoc($result)){
		echo "<tr align='center'>
					<td> <input type='checkbox' name='HW[]' id='HW[]' value='".$row['homework_id']."'></td>
					<td>  <a href='homeworkgrade.php?B=".$row['homework_id']."' target='right' >".$row['homework_id']."</a></td>
...
?>
		...
		<?php
			echo "<input type='submit' name='submit' value='測試' style='font-size:18px target='right' onclick=\"form.action='homeworktopic.php'\">";
		?>
		<input type="submit" name="submit" value="刪除" style="font-size:18px" formtarget="_parent" onclick="form.action='DeleteHW.php'">	
	</form><br>
</html>